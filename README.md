...
And creates .gcode files which can be sent to a 3D printer. Do take note that this slicer creates gcode for the Bambu Lab A1 printer.
The start and end code are thus specialised for the A1 printer. However these should be easily modified by changing the start and end gcode file
in the resources directory.

![AI generated brioche slicer project banner](briocheSlicer/Docs/brioche_cover_gemini.png)

This project is a Visual Studio solution which uses the following NuGet packages:
- Clipper2: used for the toolpath generations and the boolean set operations on the path regions.
- HelixToolkit WPF: used to create the user interface and visualize the 3D models as well as the slice view.
- HdbscanSharp: used for hierarchical clustering of the seeds to efficiently calculate the paths for the TRUNK support.
- MeshLib: used for boolean set operations on the 3D meshes of the object and the trunk support.
...
In the following sections we will give a high level overview of the workings of the project.
This is mainly so the teaching staff can easily move to critical functions.

## Docs/
The docs directory contains the prompts sent to AI, as requested by the teaching staff.
...
It is here in the _MainWindow.Slice_Click()_ where the slicer object gets created and where the 3D model and print settings turn into a sliced brioche model.
...
be fed into the code generation for printing. The slicing starts in the _TheSlicer.Slice_Model()_ where the model gets divided in a variable number of slices. 
To calculate the number of slices the model bounds and layer height are used. 

With the model divided in slices, we calculate the perimeter of the model at each slice using the _TheSlicer.Slice_Plane()_ function. 
This function implements the **first of the three stage** slicing project. The model consists of a list of triangles, each triangle consists of three edges.
...
resulting in a 2D perimeter slice at each Z value of the slicing plane. At this stage, a slice only consists of a **perimeter** which are created with the 
_BriocheSlice.Generate_Shells()_ function.

The first stage resulted in a list of **BriocheSlice** where only the perimeter was set. This ordered list of slices is used as input for the **BriocheModel**. The constructor
...
_BriocheModel.Downwards_Pass()_. In the upwards pass the **floors** are being generated using the _BriocheSlice.Generate_Floor()_ function. The downwards pass creates the 
...
## Gcode/
The gcode directory contains all the code that is associated with the generation of the gcode.
...
## Resources/
The resources directory holds the start and end code to append to the generated gcode, it also hosts the collection of used test models.
